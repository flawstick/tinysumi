...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create form data for token request - include client_id and client_secret in the body
    const params = new URLSearchParams({
      client_id: clientId,
      client_secret: clientSecret,
      grant_type: "authorization_code",
      code,
      redirect_uri: "tinysumi://",
    });

    // Send request with credentials in form body (alternative to basic auth)
    ...
    if (!tokenResponse.ok) {
      ...
      console.error(
        `Token error: ${tokenResponse.status}, Details: ${errorText}`,
      );
      return NextResponse.json(
    ...
    if (!userResponse.ok) {
      ...
      console.error(
        `User info error: ${userResponse.status}, Details: ${errorText}`,
      );
      return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    // If no user exists with this username, return an error
    if (existingUsers.length === 0) {
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }

    // Use existing user
    const userRecord = existingUsers[0] as User;
